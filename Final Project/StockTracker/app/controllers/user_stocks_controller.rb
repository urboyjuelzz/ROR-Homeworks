class UserStocksController < ApplicationController
  ...
  def create
    # if the entered :stock_id is present?
    if params[:stock_id].present?
      # Create a new UserStock where the stock_id value is whats entered as the params for the current_user.
      @user_stock = UserStock.new(stock_id: params[:stock_id], user: current_user)
    # if entered stock_id is not present?
    else
      # creating a new var called stock which tries to find by ticker entered by user.
      stock = Stock.find_by_ticker(params[:stock_ticker])
      if stock #if stock ticker is valid (in the database)
        # create a new stock stock for the current user where the value of stock: key is stock
